...
- 🟡 Stage 1 — Static HTML skeleton: `index.html` + `css/style.css` + `js/main.js` with plain "Happy Birthday [NAME HERE]" text, no 3D or animation yet. Name populated from `?name=` URL param with fallback. **Pause for review.**
...
- Consider adding a subtle background music toggle (birthday jingle)
- OG/meta tags so shared links preview nicely on social/messaging apps
- Accessibility pass: ensure animation respects `prefers-reduced-motion`
- Consider a "Send a card" share-link generator UI on the page itself
